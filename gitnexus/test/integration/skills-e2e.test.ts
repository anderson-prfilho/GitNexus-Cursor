 ...
 * 1. CLI exits 0
 * 2. .gitnexus/ exists
 * 3. >= minSkills SKILL.md files under .cursor/skills/
...
function assertSkillFiles(
  ...
  const generatedDir = path.join(tmpDir, '.cursor', 'skills');
...
/**
 * Assert AGENTS.md contains generated skill references when skills exist.
 * Automatically detects whether skills were generated by checking for
 * the community skill output directory.
 */
function assertContextFiles(result: ReturnType<typeof runSkillsCli>, tmpDir: string) {
  ...
  if (skillsGenerated) {
    ...
    expect(agentsContent).toContain('.cursor/skills/');
  }
}
